...
def automate():
    ...
    if bdfr_cfg.get('users') is not None:
        ...
        while True:
            for user in bdfr_cfg.get('users'):
                ...
                if bdfr_cfg['run_bdfr']:
                    ...
                    subprocess.call(["python3.9", "-m", "bdfr", "archive", "--user", "me", "--saved", "-L", str(bdfr_cfg['limit']),
                                    "--authenticate", input_folder, "--config", bdfr_config_file])
                    subprocess.call(["python3.9", "-m", "bdfr", "download", "--user", "me", "--saved", "-L", str(bdfr_cfg['limit']),
                                    "--exclude-id-file", idList, "--authenticate", "--file-scheme", "{POSTID}", input_folder, "--config", bdfr_config_file])
                if not merge_users:                     
                    subprocess.call(["python3.9", "-m", "bdfrtohtml", "--config", config_path, "--input_folder", input_folder, "--output_folder", output_folder])
            if merge_users:                     
                subprocess.call(["python3.9", "-m", "bdfrtohtml", "--config", config_path, "--input_folder", input_folder, "--output_folder", output_folder])
        ...
        while True:
            ...
            subprocess.call(["python3.9", "-m", "bdfrtohtml", "--config", config_path])
            logging.info(f"Runs complete, now waiting for {int(bdfr_cfg['frequency'])} minutes before next run.")
            time.sleep(int(bdfr_cfg['frequency'])*60)
